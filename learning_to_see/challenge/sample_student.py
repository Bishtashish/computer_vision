...
#     treeRule1 = lambda X: np.logical_and(np.logical_and(X[:, 13] == 1, X[:,0] == 0), X[:, 16] == 0)
    yhat = treeRule1(X)
    yhat_reshaped = reshapeIntoImage(yhat, im.shape, s=9)
    splicedRow = yhat_reshaped[yhat_reshaped.shape[0]//4,:]
    bool_list = list(map(bool,splicedRow))
#     return countEdges(splicedRow.tolist())
    count = sum([1 if (i ==0 and splicedRow[i] == True ) or (splicedRow[i-1]==False and splicedRow[i]==True) else 0 for i,n in enumerate(bool_list)])
    #count= [sum(1) for i,n in enumerate(bool_list) if(n==True and bool_list[i-1]==False)]
    print(count)
    return count
